...
def make_fibonacci_lattice(N, show=False):
  import math
  golden_ratio = (1. + math.sqrt(5.)) / 2.
  # calculate latitude and longtitude
  idx = np.arange(-N, N+1).astype(np.float64)
  lat = np.arcsin((2*idx) / (2*N+1))
  lon = idx * 2. * math.pi / golden_ratio 
...
def to_spind_file(index_stats, filename, offset=0.0):
  """Write abc star to file in spind format, add offset if given
  
  Args:
    index_stats (TYPE): Description
    filename (TYPE): Description
    offset (float, optional): Relative offset to length of xstar.
  ...
  for c in chunks:
    if c.indexed_by == 'none':
      ...
      cr = c.crystal
      xs, ys, zs = make_fibonacci_lattice(1000)
      random_id = random.randint(0, xs.size-1)
      x, y, z = xs[random_id], ys[random_id], zs[random_id]
      random_vec = np.asarray([x, y, z])
      print('random vector: ', random_vec)
      astar = cr.astar + offset * norm(cr.astar) * random_vec
      bstar = cr.bstar + offset * norm(cr.bstar) * random_vec
      cstar = cr.cstar + offset * norm(cr.cstar) * random_vec
